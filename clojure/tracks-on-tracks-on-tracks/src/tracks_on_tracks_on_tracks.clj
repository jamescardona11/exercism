...
(defn remove-language
  ...
  [lang-list]
  (remove #{(first-language lang-list)} lang-list)
  )
...
(defn learning-list
  ...
  of languages."
  []
  (count (add-language (add-language (remove-language (add-language (add-language (new-list) "Clojure") "Lisp")) "Java") "JavaScript"))
  )